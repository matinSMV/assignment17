...
class Window(QMainWindow):
    def __init__(self):
        ...
        self.ui.show()
        self.ui.about_btn.setStyleSheet('color: white ; background-color: rgb(85, 170, 0)')
        self.ui.restart_btn.setStyleSheet('color: white; background-color: rgb(85, 170, 0)')
        self.AI = False
        ...
        self.ui.restart_btn.clicked.connect(partial(self.restart))
        self.turn = 'X'
        self.ui.rd_btn2.setChecked(True)

    def about(self):
        msgBox = QMessageBox()
        msgBox.setText('type: fun simple game\ngame name: Tic Tac Toe\nDesigned By: Matin Samavatian')
        msgBox.exec()
    ...
    def set_position(self, i, j):
        if self.turn == 'X':
            ...
            self.game[i][j].setStyleSheet('color: blue; background-color: lightgrey')
        else:
            self.turn = 'X'
    ...
    def restart(self, score_reset=True):
        for i in range(3):
            for j in range(3):
                self.game[i][j].setText('')
